...
export default function ModeratorSidebar() {
  ...
  const navLinks = [
    { name: 'Dashboard', href: '/moderator/dashboard', icon: 'dashboard' },
    { name: 'Grocery Orders', href: '/moderator/orders', icon: 'local_shipping' },
    { name: 'Zone Customers', href: '/moderator/users', icon: 'groups' },
    { name: 'Grocery Products', href: '/moderator/products', icon: 'storefront' },
    { name: 'Categories', href: '/moderator/categories', icon: 'category' }
  ];
      ...
      <nav className="flex-1 py-4 flex flex-col gap-1 px-3 overflow-y-auto">
        {navLinks.map((link) => {
          ...
          return (
            ...
            >
              <span className={`material-symbols-outlined text-[20px] transition-transform duration-200 ${isActive ? 'scale-110' : ''}`}>{link.icon}</span>
              {link.name}
              {isActive && (
      ...
      <div className="p-4 border-t border-slate-100 shrink-0">
        <div className="bg-slate-50 border border-slate-100 rounded-2xl p-4">
            ...
            <div className="flex items-center gap-2 mb-3">
              <span className="material-symbols-outlined text-[16px] text-emerald-600">shield_person</span>
