...
export default function RestaurantDetailClient({
  ...
  const [loading, setLoading] = useState<boolean>(!initialRestaurant && (!initialMenuItems || initialMenuItems.length === 0));
  ...
  // Background refresh for live status / fresh item pricing without blocking SSR initial paint
  useEffect(() => {
        ...
        <div className="flex gap-3 justify-center mb-4">
          <div className="flex items-center gap-1.5 px-4 py-2 rounded-full bg-[#faeddf] border border-[#f3dcb8]">
            ...
            <div className="flex flex-col items-center">
              <span className="text-[11px] font-bold text-[#a73a00] leading-none">
            ...
            <div key={`category-${category}`} id={`category-${category}`} className="scroll-mt-36">
              ...
              <div className="space-y-5">
                {(groupedItems[category] || []).map((item: any) => (
                  <div
                    key={item._id}
                    className={`bg-white rounded-2xl overflow-hidden shadow-[0_4px_12px_rgba(0,0,0,0.04)] border border-slate-100 ${
